...
const LeaveModel = {
  ...
  getEmployeeLeaveList: async () => {
    const leaveListSql = "SELECT L.id AS id, U.user_name ,U.gender, L.type_id AS typeId, LT.name as type, U.user_role, U.avatar, DATE_FORMAT(L.start, '%Y/%m/%d') AS start, DATE_FORMAT(L.end, '%Y/%m/%d') AS end, DATEDIFF(L.end, L.start) + 1 AS duration FROM `employee_leaves` AS L JOIN leave_type AS LT ON LT.id = L.type_id JOIN users AS U ON U.id = L.user_id WHERE DATE(start) BETWEEN DATE(CURRENT_DATE - INTERVAL DAYOFYEAR(CURRENT_DATE) DAY) AND start"
  ...
  leavedaysListBetweenTowDate: async (startDate, endDate) => {
    const getList = `SELECT U.user_name , U.avatar,EL.id AS elId,U.gender, EL.type_id AS typeId, LT.name as type ,DATE_FORMAT(start,'%Y/%m/%d') AS start, DATE_FORMAT(end, '%Y/%m/%d') AS end, DATEDIFF(end,start) + 1 AS duration  FROM employee_leaves AS EL
  ...
  leavedaysListBetweenTowDateWithId: async (id, startDate, endDate) => {
    const getList = `SELECT U.id as userID ,U.user_name, U.avatar,U.gender, EL.id AS elId, EL.type_id AS typeId, LT.name as type ,DATE_FORMAT(start,'%Y/%m/%d') AS start, DATE_FORMAT(end, '%Y/%m/%d') AS end, DATEDIFF(end,start) + 1 AS duration FROM employee_leaves AS EL JOIN leave_type AS LT ON LT.id = El.type_id JOIN users AS U ON U.id = EL.user_id WHERE EL.user_id = ${id} AND DATE(start) BETWEEN  '${startDate}' AND '${endDate}'`;
    console.log(getList)
    const [rows] = await dbConnect.promise().execute(getList)
    return rows;
  },
  anEmployeeLeaveList: async (id) => {
    const query = `SELECT U.user_name ,U.gender,  U.user_role,U.gender, U.avatar, EL.user_id AS id,EL.id AS elId, EL.type_id AS typeId, LT.name as type , DATE_FORMAT(start,'%Y/%m/%d') AS start, DATE_FORMAT(end, '%Y/%m/%d') AS end, DATEDIFF(end,start) + 1 AS duration  FROM employee_leaves AS EL JOIN leave_type AS LT ON LT.id = EL.type_id JOIN users AS U ON U.id = EL.user_id WHERE EL.user_id = ${id}`;
